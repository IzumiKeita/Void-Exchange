  ...
  <div class="vault-container">
    <div class="header-section">
      <div class="title-group">
        <h1>{{ t('vault.title') }}</h1>
        <div class="vault-nav">
          <router-link to="/vault" class="vault-nav-btn active">{{ t('vault.nav.warframes') }}</router-link>
          <router-link to="/vault/weapons" class="vault-nav-btn">{{ t('vault.nav.weapons') }}</router-link>
        </div>
      </div>
      <div class="controls">
        <button @click="$router.push('/')" class="btn-back">⬅ {{ t('dashboard.title') }}</button>
      </div>
    </div>
    ...
    <div v-else class="vault-content">
      ...
      <section class="vault-section active-section">
        <div class="section-header">
          ...
          <span class="badge active-badge">{{ vaultData.active.length }} {{ t('trends.table.item') }}s</span>
        </div>
        <p class="section-desc">{{ t('vault.sections.active.desc') }}</p>
      ...
      <section class="vault-section resurgence-section" v-if="vaultData.resurgence.length > 0">
        <div class="section-header">
          ...
          <span class="badge resurgence-badge">{{ vaultData.resurgence.length }} {{ t('trends.table.item') }}s</span>
        </div>
        <p class="section-desc">{{ t('vault.sections.resurgence.desc') }}</p>
      ...
      <section class="vault-section vaulted-section">
        <div class="section-header">
          ...
          <span class="badge vaulted-badge">{{ vaultData.vaulted.length }} {{ t('trends.table.item') }}s</span>
        </div>
        <p class="section-desc">{{ t('vault.sections.vaulted.desc') }}</p>
        
        <!-- Buscador para Vaulted -->
        <input v-model="searchVaulted" :placeholder="t('vault.searchPlaceholder')" class="search-input">
...
export default {
  ...
  setup() {
    ...
    const API_URL = 'http://localhost:5001/api'; // Puerto 5001

    const loadVaultData = async () => {
      try {
        loading.value = true;
        const response = await fetch(`${API_URL}/vault`);
        if (!response.ok) throw new Error('Error cargando datos de bóveda');
    ...
    const getImageUrl = (thumb) => {
      ...
      // Si no, construir la URL al proxy local
